...
// /proc is a virtual filesystem in linux that provides an interface to kernels view of processes. It is not stored on disk but is generated by the kernel at runtime. It contains information about processes, memory, devices, and other system information. Each process has a directory in /proc with its PID as the name, and it contains various files that provide information about the process such as its status, memory usage, and command line arguments.
